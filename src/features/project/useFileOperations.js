    ...
    const handleSaveToLocal = useCallback(() => {
        try {
            ...
            const payload = {
                ...
                editorPrefs: {
                    fontSize: Math.min(Math.max(Number(state.editorPrefs.fontSize) || 14, 12), 20),
                    lineHeight: Math.min(Math.max(Number(state.editorPrefs.lineHeight) || 1.6, 1.2), 2.1),
                    autoPreview: Boolean(state.editorPrefs.autoPreview),
                },
                workspacePrefs: {
                    previewMode: state.workspacePrefs.previewMode === 'mobile' ? 'mobile' : 'desktop',
                },
        ...
        try {
            ...
            const sanitizeFilename = (name) => {
                return name
                    ...
                    .slice(0, 100) || 'project'              // Limit length
            }
